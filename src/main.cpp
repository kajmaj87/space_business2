...
auto main(int argc, const char *argv[]) -> int {
  ...
  auto renderer_orbiter = Renderer([&] {
    auto c = Canvas(100, 100);
    c.DrawText(0, 0, "Mousecentric solar system");

    c.DrawPoint(mouse_x, mouse_y, true);
    c.DrawPoint(mouse_x + std::sin(rotation) * 30, mouse_y + std::cos(rotation) * 30,
                true);
    // c.DrawPoint(mouse_x+sin(rotation)* 5, mouse_y, true);
    // c.DrawPoint(mouse_x, mouse_y+cos(rotation)*5, true);
    // c.DrawPoint(mouse_x, mouse_y+cos(rotation)*5, true);
  ...
  auto tab = Container::Tab(
      {renderer_line_braille, renderer_line_block, renderer_circle_braille,
       renderer_circle_block, renderer_circle_filled_braille,
       renderer_circle_filled_block, renderer_ellipse_braille,
       renderer_ellipse_block, renderer_ellipse_filled_braille,
       renderer_ellipse_filled_block,

       renderer_plot_1, renderer_plot_2, renderer_plot_3,

       renderer_text, renderer_orbiter},
      &selected_tab);
  ...
  component_renderer = CatchEvent(component_renderer, [&](const Event& event) {
    if (event.is_mouse()) {
      rotation += 0.1;
    }
    return false;
  ...
  auto gameLoop = [&screen, &rotation](double delta) {
    while (true) {
